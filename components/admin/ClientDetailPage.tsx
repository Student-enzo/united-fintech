...
import {
  ...
  Upload, ExternalLink, AlertCircle, ChevronRight, CheckCircle2, Circle,
} from 'lucide-react'
...
const DOC_TYPES = ['Articles of Inc', 'Voided Check', 'Driver License', 'Bank Statement', 'Other'] as const
type DocType = typeof DOC_TYPES[number]

const REQUIRED_DOCS: { label: string; key: DocType; required: boolean }[] = [
  { label: 'Government ID',             key: 'Driver License',  required: true  },
...
const SectionHeader = ({ children }: { children: React.ReactNode }) => (
  <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: BRAND.cyan, marginBottom: 12 }}>
    {children}
  </div>
)
...
function AddAccountModal({ clientId, onClose, onAdded }: {
  clientId: string; onClose: () => void; onAdded: (a: MerchantAccount) => void
}) {
  const [form, setForm] = useState({
    account_name: '', account_type: 'Card Present' as MerchantAccount['account_type'],
    mcc: '', mcc_label: '', risk: 'low' as RiskLevel,
  })
  ...
  const inp: React.CSSProperties = {
    ...
    borderRadius:6, padding:'8px 10px', fontSize:13, color:BRAND.silver, outline:'none', boxSizing:'border-box',
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const base = {
      ...
      pipeline_stage: 'lead_identified' as PipelineStage,
      days_in_stage:0, monthly_volume:0, avg_ticket:0, card_present_pct:0,
      ...
      <div onClick={e=>e.stopPropagation()} style={{ backgroundColor:BRAND.card, border:`1px solid ${BRAND.border}`, borderRadius:14, padding:28, width:460, maxWidth:'90vw', boxShadow:'0 24px 64px rgba(0,0,0,0.7)' }}>
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div><label style={lbl}>Account Type</label>
              <select style={{...inp,cursor:'pointer'}} value={form.account_type} onChange={e=>setForm(f=>({...f,account_type:e.target.value as MerchantAccount['account_type']}))}>
                {(['Card Present','eCommerce','MOTO','ACH'] as const).map(t=><option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div><label style={lbl}>Risk</label>
...
function UploadZone({ clientId, onUploaded }: { clientId: string; onUploaded: (d: ClientDocument) => void }) {
  ...
  const handle = useCallback((files: FileList|null) => {
    if (!files) return
    Array.from(files).forEach(f => onUploaded({
      id: `upload-${Date.now()}-${Math.random().toString(36).slice(2)}`, client_id: clientId,
      file_name: f.name, file_url: URL.createObjectURL(f), doc_type: docType,
      file_size: f.size, uploaded_at: new Date().toISOString(),
    }))
  ...
  return (
    <div style={{ marginTop:12 }}>
      ...
      <div onClick={()=>inputRef.current?.click()}
        ...
        onDrop={(e:DragEvent<HTMLDivElement>)=>{e.preventDefault();setDragOver(false);handle(e.dataTransfer.files)}}
...
function DocRow({ doc }: { doc: ClientDocument }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', borderRadius:7, backgroundColor:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.06)' }}>
      ...
      <a href={doc.file_url} target="_blank" rel="noopener noreferrer" style={{ color:BRAND.cyan, flexShrink:0, cursor:'pointer' }}><ExternalLink size={13}/></a>
    </div>
  )
}
  ...
  return (
    <div style={{ padding:'32px 40px', maxWidth:1200 }}>
      ...
      <div style={{ display:'flex', gap:32, marginTop:32 }}>
        ...
        <div style={{ flex:2, display:'flex', flexDirection:'column', gap:16 }}><Skeleton h={160}/><Skeleton h={220}/></div>
      </div>
    </div>
  )
}
...
export default function ClientDetailPage({ clientId }: { clientId: string }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        setClient(c); setNotes(c.notes??''); setAccounts(c.merchant_accounts??[]); setDocuments(docs)
      } catch (e) {
  ...
  const handleNoteBlur = useCallback(async () => {
    ...
    try {
      ...
      await supabasePublic.from('clients').update({ notes }).eq('id', client.id)
    } catch (e) { console.warn('[ClientDetailPage] notes save failed:', e) }
  }, [client, notes])
    ...
    <div style={{ padding:'28px 40px', maxWidth:1280, margin:'0 auto' }}>
      ...
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', marginTop:10, gap:10 }}>
          ...
          <span style={{ display:'inline-block', padding:'2px 8px', borderRadius:5, fontSize:11, color:daysOnFile>30?BRAND.warn:BRAND.muted, backgroundColor:daysOnFile>30?'rgba(252,211,77,0.08)':'rgba(255,255,255,0.05)', border:`1px solid ${daysOnFile>30?'rgba(252,211,77,0.2)':'rgba(255,255,255,0.08)'}` }}>{daysOnFile}d in stage</span>
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:32, alignItems:'flex-start' }}>
        ...
        <div style={{ flex:3, minWidth:0 }}>
          <SectionHeader>Account Pipeline</SectionHeader>
          {accounts.length > 0
          ...
          <button
            ...
            onMouseEnter={e=>(e.currentTarget.style.backgroundColor='rgba(144,196,207,0.06)')}
        ...
        <div style={{ flex:2, minWidth:0, display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {client.legal_structure && (
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:13 }}>
                  <span style={{ color:BRAND.muted }}>Legal Structure</span>
                  <span style={{ color:BRAND.silver }}>{client.legal_structure}</span>
                </div>
              )}
              {(client.partner||client.partner_iso) && (
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:13 }}>
                  <span style={{ color:BRAND.muted }}>Partner ISO</span>
                  <span style={{ color:BRAND.silver }}>{client.partner_iso?`[${client.partner_iso}] `:''}{client.partner}</span>
                </div>
              )}
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, alignItems:'center' }}>
                <span style={{ color:BRAND.muted }}>Risk Level</span>
                <span style={{ display:'flex', alignItems:'center', color:BRAND.silver }}>
                  <RiskDot risk={accounts[0]?.risk}/>
                  {((accounts[0]?.risk??'low').charAt(0).toUpperCase()+(accounts[0]?.risk??'low').slice(1))}
                </span>
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:13 }}>
                <span style={{ color:BRAND.muted }}>Date Added</span>
                <span style={{ color:BRAND.silver }}>{fmt.date(client.date_added)}</span>
              </div>
              <div style={{ marginTop:4 }}>
                <div style={{ fontSize:11, color:BRAND.muted, marginBottom:4, fontWeight:600, letterSpacing:'0.04em' }}>NOTES</div>
                <textarea
                  value={notes} onChange={e=>setNotes(e.target.value)} onBlur={handleNoteBlur}
                  placeholder="Internal notes…" rows={3}
                  style={{ width:'100%', backgroundColor:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:6, padding:'7px 9px', fontSize:12, color:BRAND.silver, resize:'vertical', outline:'none', lineHeight:1.5, boxSizing:'border-box', fontFamily:'inherit' }}
                />
              </div>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, fontWeight:700, color:BRAND.muted, letterSpacing:'0.06em', textTransform:'uppercase', marginBottom:8 }}>Required Documents</div>
                ...
                {REQUIRED_DOCS.map(rd => {
                  ...
                  return (
                    <div key={rd.key} style={{ display:'flex', alignItems:'center', gap:8, fontSize:12 }}>
                      ...
                      <span style={{ flex:1, color: submitted ? BRAND.silver : rd.required ? BRAND.danger : BRAND.muted }}>
                        {rd.label}
                      </span>
                      <span style={{
                        ...
                        ...(submitted
                          ...
                          : rd.required
                          ? { color:BRAND.danger, backgroundColor:'rgba(232,80,74,0.08)', border:'1px solid rgba(232,80,74,0.22)' }
                          : { color:BRAND.muted, backgroundColor:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)' }
                        ),
                      }}>
            ...
            {documents.length > 0
              ...
              : <div style={{ fontSize:12, color:BRAND.muted, marginBottom:12, textAlign:'center', padding:'8px 0' }}>No documents uploaded yet.</div>
            }
