...
// @ts-ignore — no types for svg-path-commander
...
const Gravity = forwardRef<GravityRef, GravityProps>(
  (
    ...
    const registerElement = useCallback(
      (id: string, element: HTMLElement, props: MatterBodyProps) => {
        ...
        if (props.bodyType === 'circle') {
          ...
          body = Bodies.circle(x, y, radius, { ...(props.matterBodyOptions as Matter.IChamferableBodyDefinition), angle, render: renderOpts })
          ...
          body = Bodies.fromVertices(x, y, vertexSets, { ...(props.matterBodyOptions as Matter.IChamferableBodyDefinition), angle, render: renderOpts })
        } else {
          body = Bodies.rectangle(x, y, width, height, { ...(props.matterBodyOptions as Matter.IChamferableBodyDefinition), angle, render: renderOpts })
        }
        ...
        Events.on(engine.current, 'beforeUpdate', () => {
          if (canvas.current) {
            canvas.current.style.cursor = !mouseDown.current && !touchingMouse()
              ? 'default'
              : touchingMouse()
              ? mouseDown.current ? 'grabbing' : 'grab'
              : 'default'
          }
        })
    ...
    useImperativeHandle(ref, () => ({ start: startEngine, stop: stopEngine, reset }), [startEngine, stopEngine])
